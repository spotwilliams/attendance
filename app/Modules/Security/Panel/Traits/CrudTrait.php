...
trait CrudTrait
{
    ...
    */
    ...
    public static function getPossibleEnumValues($field_name)
    {
        ...
        $type = \Illuminate\Support\Facades\DB::select(\Illuminate\Support\Facades\DB::raw('SHOW COLUMNS FROM '.$instance->getTable().' WHERE Field = "'.$field_name.'"'))[0]->Type;
    ...
    {
        $instance = new static(); // create an instance of the model to be able to get the table name
        $answer = \Illuminate\Support\Facades\DB::select(\Illuminate\Support\Facades\DB::raw("SELECT IS_NULLABLE FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME='".$instance->getTable()."' AND COLUMN_NAME='".$column_name."' AND table_schema='".env('DB_DATABASE')."'"))[0];
